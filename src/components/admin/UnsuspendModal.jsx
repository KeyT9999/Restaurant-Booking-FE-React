  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-[#1A1D24] border border-zinc-800 rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-lg font-bold text-zinc-150 mb-2">Gỡ tạm ngưng nhà hàng</h3>
        <p className="text-sm text-zinc-400 mb-2">Bạn có chắc muốn gỡ tạm ngưng và khôi phục hoạt động cho nhà hàng <strong className="text-zinc-200">{restaurant.name}</strong>?</p>
        <p className="text-xs text-zinc-500 mb-4">Nhà hàng sẽ tiếp tục được hiển thị công khai và nhận đặt bàn.</p>
