...
const OWNER_PROMPTS = [
  ...
  'Bàn nào còn trống lúc 19h?',
  'Khách nào sắp đến?',
  'Tóm tắt review tuần này',
];
...
const getToolErrorMessage = (data) => {
  if (data?.errorCode === 'SELECTED_RESTAURANT_REQUIRED') {
    return 'Vui lòng chọn nhà hàng trước khi dùng Trợ lý AI.';
  }
  if (data?.errorCode === 'OWNER_RESTAURANT_FORBIDDEN') {
    return 'Bạn không có quyền xem dữ liệu của nhà hàng này.';
  }
  if (['TOOL_NOT_ALLOWED', 'AUTH_REQUIRED'].includes(data?.errorCode)) {
    return 'Tài khoản hiện tại không có quyền dùng trợ lý AI dành cho chủ nhà hàng.';
  }
  return data?.message || 'Không thể tải dữ liệu owner cho yêu cầu này.';
};

const isPermissionError = (code) => (
  ['TOOL_NOT_ALLOWED', 'AUTH_REQUIRED', 'OWNER_RESTAURANT_FORBIDDEN'].includes(code)
);

export default function OwnerAIWidget() {
    ...
    <div className="fixed inset-x-3 bottom-28 z-[1190] flex justify-end font-sans pointer-events-none sm:left-auto sm:right-6">
      {!isOpen && (
        <button
          ...
          aria-label="Mở Trợ lý Owner AI"
          aria-expanded="false"
          aria-controls="owner-ai-panel"
        >
          <Sparkles size={18} aria-hidden="true" />
          <span className="hidden sm:inline">Owner AI</span>
        </button>
      )}
      ...
      {isOpen && (
        <div className="pointer-events-auto">
          <OwnerAIWidgetPanel
            key={selectedRestaurantId || 'no-selected-restaurant'}
...
function OwnerAIWidgetPanel({
            ...
            setMessages((current) => current.map((item) => (
              item.id === assistantMessageId
                ? {
                  ...
                  toolStatus: {
                    tool: data.tool,
                    label: data.label || 'Đang tải dữ liệu owner...',
                    status: 'running',
                  },
                }
                : item
          ...
          if (event === 'tool_completed') {
            ...
            if (failed) {
              ...
              setError({
                ...
                retry: permission || data.errorCode === 'SELECTED_RESTAURANT_REQUIRED' ? null : {
                  message,
                  assistantMessageId,
                  userMessageId,
                  history,
  ...
  const requestAssistant = (rawMessage) => {
    ...
    if (!ownerContext) {
      setError({ message: 'Vui lòng chọn nhà hàng trước khi dùng Trợ lý AI.', tone: 'permission', retry: null });
      return;
    }
  ...
  return (
        <section
          id="owner-ai-panel"
          className="flex h-[560px] max-h-[calc(100vh-8rem)] w-full max-w-full flex-col overflow-hidden rounded-xl border border-border bg-card shadow-2xl sm:w-[420px]"
          aria-label="Trợ lý AI chủ nhà hàng"
        >
          <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-[#14171D] px-4">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <h2 className="truncate text-sm font-semibold text-foreground">Trợ lý AI chủ nhà hàng</h2>
                <p className="truncate text-xs text-muted-foreground">
                  {selectedRestaurant?.name || 'Dữ liệu vận hành read-only'}
                </p>
              </div>
            </div>
            <button
              ...
              onClick={onClose}
              aria-label="Đóng Trợ lý Owner AI"
            >
          ...
          >
            {messages.length === 0 && (
              <div className="my-auto text-center">
                ...
                <h3 className="text-base font-semibold text-foreground">Bạn muốn xem gì?</h3>
                <p className="mx-auto mt-1 max-w-[34ch] text-sm text-muted-foreground">
                  Trợ lý chỉ đọc dữ liệu của nhà hàng đang chọn và trả về tóm tắt an toàn.
                </p>

                {restaurantLoading && (
                  <p className="mt-3 rounded-lg border border-border bg-secondary/50 p-2.5 text-xs text-muted-foreground">
                    Đang tải danh sách nhà hàng...
                  </p>
                )}

                {requiredState && (
                  <p className="mt-3 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-2.5 text-left text-xs leading-relaxed text-amber-100">
                    <ShieldAlert size={14} className="mt-0.5 shrink-0" aria-hidden="true" />
                    <span>Vui lòng chọn nhà hàng trước khi dùng Trợ lý AI.</span>
                  </p>
                )}

                {restaurantError && (
                  <p className="mt-3 rounded-lg border border-destructive/30 bg-destructive/10 p-2.5 text-xs text-destructive">
                    {restaurantError}
                  </p>
                )}

                <div className="mt-4 flex flex-wrap justify-center gap-2">
                  {OWNER_PROMPTS.map((prompt) => (
                    <button
                      type="button"
                      key={prompt}
          ...
          <form className="shrink-0 border-t border-border bg-card p-3" onSubmit={handleSubmit}>
            <label className="sr-only" htmlFor="owner-ai-message">Tin nhắn cho Trợ lý AI chủ nhà hàng</label>
            <div className="flex items-end gap-2">
              ...
              {sending ? (
                ...
                <button
                  ...
                  aria-label="Gửi tin nhắn Owner AI"
                >
